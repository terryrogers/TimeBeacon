...
async function preloadClockImage(zone){
    try{
        const {response,payload}=await requestData('/dashboard/clocks/image?zone='+encodeURIComponent(zone),{cache:'no-store',signal:AbortSignal.timeout(20000)});
        if(!response.ok)return false;
        const {image:photo}=payload;
        if(!photo?.image_url)return false;
        return await withLoading(()=>new Promise(resolve=>{
        ...
        }),{target:document.getElementById('user-clocks'),label:'Preparing City Background…'});
    }catch{return false;}
}
async function loadClockCatalogue(){if(!clockCatalogue){const {response,payload}=await requestData('/static/timezones.json');if(!response.ok)throw Error('The clock list could not be loaded. Reload and try again.');clockCatalogue=payload.zones.map(clock=>({...clock,location:clock.location.replace(/\bGMT\s*(?=[+-])/g,'UTC ').replace(/\bGMT\b/g,'UTC')}));}return clockCatalogue;}
