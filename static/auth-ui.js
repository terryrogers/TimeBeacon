async function initialiseSignIn(){
    ...
    submit('login-form',async form=>{const body=values(form);form.elements.password.value='';const result=await accessRequest('/auth/login','POST',body);if(result.stage==='complete')return complete();stage(result.stage);if(result.stage==='enroll'){const enrollment=await accessRequest('/auth/enrollment/start','POST',{});$('signin-enrollment-qr').src=enrollment.qr;$('signin-enrollment-secret').textContent=enrollment.secret;}},'login-feedback');
    submit('factor-form',async form=>{const body=values(form);form.reset();await accessRequest('/auth/second-factor','POST',body);await complete();},'login-feedback');
    submit('recovery-form',async form=>{const body=values(form);form.reset();await accessRequest('/auth/second-factor','POST',{...body,recovery:true});await complete();},'login-feedback');
